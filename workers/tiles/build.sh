...
OTHERS_MB=$(( LIMIT_MB * (BUDGET_CONTOURS_PCT + BUDGET_TERRAIN_PCT + BUDGET_TRAILS_PCT + BUDGET_FEATURES_PCT + BUDGET_TRANSPORT_PCT + BUDGET_BOUNDARIES_PCT + BUDGET_WATER_PCT) / 100 + BUDGET_ASSETS_MB ))
BUDGET_MB=$(( LIMIT_MB - OTHERS_MB ))
echo "Rozpočet stránky ${LIMIT_MB} MB − vrstevnice ${BUDGET_CONTOURS_PCT} % − terén ${BUDGET_TERRAIN_PCT} % − trasy ${BUDGET_TRAILS_PCT} % − krajinné prvky ${BUDGET_FEATURES_PCT} % − dopravná sieť ${BUDGET_TRANSPORT_PCT} % − hranice ${BUDGET_BOUNDARIES_PCT} % − vodstvo ${BUDGET_WATER_PCT} % − ikonky a fonty ${BUDGET_ASSETS_MB} MB = ${BUDGET_MB} MB na dlaždice"
